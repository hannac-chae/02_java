...
public class IntDoubleWrapper {
	...
	public static void main(String[] args) {
		...
		String intFrm = String.format("문자열[%s] ==> 기본형(int)[%d]"
				, intInput, Integer.parseInt(intInput));
		
		String dbFrm = String.format("문자열[%s] ==> 기본형(double)[%f]"
		...
		// static import 를 진행하면 partseType() 메소드 호출시
		// 클래스 이름 생략
		...
		String tenFrm = String.format("%d 의 참조형 데이터 : %s"
				, ten, objTen.toString());
		String sixtyFrm = String.format("%f 의 참조형 데이터 : %s"
				, sixty, objSixty.toString());
